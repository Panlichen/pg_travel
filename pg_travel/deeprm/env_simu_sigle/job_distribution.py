...
def generate_sequence_work(hp, seed=42):

    np.random.seed(seed)

    simu_len = hp.simu_len * hp.num_ex

    nw_dist = hp.dist.bi_model_dist

    nw_len_seq = np.zeros(simu_len, dtype=int)
    nw_size_seq = np.zeros((simu_len, hp.num_res), dtype=int)

    for i in range(simu_len):

        if np.random.rand() < hp.new_job_rate:  # a new job comes

            nw_len_seq[i], nw_size_seq[i, :] = nw_dist()

    nw_len_seq = np.reshape(nw_len_seq,
                            [hp.num_ex, hp.simu_len])
    nw_size_seq = np.reshape(nw_size_seq,
                             [hp.num_ex, hp.simu_len, hp.num_res])
